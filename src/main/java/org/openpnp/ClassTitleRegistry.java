...
public class ClassTitleRegistry {
    ...
    private static final String CLASS_PREF_PREFIX = "class.";
    private static final String TEXT_PREF_PREFIX = "text.";
    ...
    static {
        ...
        // ======== 补充缺失的静态文本映射（校准、运动规划器、吸嘴创建等）========
        // VisionSolutions - Primary/Secondary calibration fiducial
        addTextMapping(new TextMapping("ID_400",
                "Primary calibration fiducial position and initial camera calibration.",
                "Primary calibration fiducial position and initial camera calibration.",
                "主校准基准点位置与初始相机校准。"));
        addTextMapping(new TextMapping("ID_401",
                "Move the camera over the primary calibration fiducial and capture its position.",
                "Move the camera over the primary calibration fiducial and capture its position.",
                "将相机移动到主校准基准点上方，并记录当前位置。"));
        addTextMapping(new TextMapping("ID_402",
                "Secondary calibration fiducial position.",
                "Secondary calibration fiducial position.",
                "辅助校准基准点位置。"));
        addTextMapping(new TextMapping("ID_403",
                "Move the camera over the secondary calibration fiducial and capture its position.",
                "Move the camera over the secondary calibration fiducial and capture its position.",
                "将相机移动到辅助校准基准点上方，并记录当前位置。"));

        // ReferenceMachine - Motion Planner
        addTextMapping(new TextMapping("ID_404",
                "Advanced Motion Planner not set. Accept or Dismiss to continue.",
                "Advanced Motion Planner not set. Accept or Dismiss to continue.",
                "尚未设置高级运动规划器。请选择接受或跳过以继续。"));
        addTextMapping(new TextMapping("ID_405",
                "Change to ReferenceAdvancedMotionPlanner",
                "Change to ReferenceAdvancedMotionPlanner",
                "切换为 ReferenceAdvancedMotionPlanner"));
        addTextMapping(new TextMapping("ID_406",
                "Advanced motion planner set. Revert to a simpler, safer planner.",
                "Advanced motion planner set. Revert to a simpler, safer planner.",
                "已设置高级运动规划器。恢复为更简单、更安全的规划器。"));
        addTextMapping(new TextMapping("ID_407",
                "Change to NullMotionPlanner",
                "Change to NullMotionPlanner",
                "切换为 NullMotionPlanner"));

        // HeadSolutions - Create Nozzles (uses Translations.getString but also needs mapping)
        addTextMapping(new TextMapping("ID_408",
                "Create nozzles for this head.",
                "Create nozzles for this head.",
                "为该贴装头创建吸嘴单元。"));
        addTextMapping(new TextMapping("ID_409",
                "Choose the number and type of your nozzles.",
                "Choose the number and type of your nozzles.",
                "选择吸嘴单元的数量和类型。"));

        // ContactProbeNozzle
        addTextMapping(new TextMapping("ID_410",
                "The nozzle can be replaced with a ContactProbeNozzle to support various probing features.",
                "The nozzle can be replaced with a ContactProbeNozzle to support various probing features.",
                "可将该吸嘴替换为 ContactProbeNozzle，以支持多种探测功能。"));
        addTextMapping(new TextMapping("ID_411",
                "Replace with ContactProbeNozzle.",
                "Replace with ContactProbeNozzle.",
                "替换为 ContactProbeNozzle。"));
        addTextMapping(new TextMapping("ID_412",
                "Converting the ContactProbeNozzle back to a plain ReferenceNozzle may simplify the machine setup.",
                "Converting the ContactProbeNozzle back to a plain ReferenceNozzle may simplify the machine setup.",
                "将 ContactProbeNozzle 转换回普通 ReferenceNozzle，可简化机器设置。"));
    ...
    public static void saveAll() {
        try {
            for (ClassMapping mapping : mappings) {
                String prefix = CLASS_PREF_PREFIX + mapping.id + ".";
                prefs.put(prefix + "englishTitle", nullToEmpty(mapping.englishTitle));
                prefs.put(prefix + "chineseTitle", nullToEmpty(mapping.chineseTitle));
                prefs.put(prefix + "category", nullToEmpty(mapping.category));
                prefs.putBoolean(prefix + "appendName", mapping.appendName);
            }
            for (TextMapping mapping : textMappings) {
                String prefix = TEXT_PREF_PREFIX + mapping.id + ".";
                prefs.put(prefix + "englishTranslation", nullToEmpty(mapping.englishTranslation));
                prefs.put(prefix + "chineseTranslation", nullToEmpty(mapping.chineseTranslation));
            }
            prefs.flush();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void loadUserOverrides() {
        for (ClassMapping mapping : mappings) {
            String prefix = CLASS_PREF_PREFIX + mapping.id + ".";
            mapping.englishTitle = prefs.get(prefix + "englishTitle", mapping.englishTitle);
            mapping.chineseTitle = prefs.get(prefix + "chineseTitle", mapping.chineseTitle);
            mapping.category = prefs.get(prefix + "category", mapping.category);
            mapping.appendName = prefs.getBoolean(prefix + "appendName", mapping.appendName);
        }
        for (TextMapping mapping : textMappings) {
            String prefix = TEXT_PREF_PREFIX + mapping.id + ".";
            mapping.englishTranslation = prefs.get(prefix + "englishTranslation", mapping.englishTranslation);
            mapping.chineseTranslation = prefs.get(prefix + "chineseTranslation", mapping.chineseTranslation);
        }
    }
    ...
    private static String getDefaultTitle(Object obj) {
        if (obj instanceof Named) {
            ...
            return obj.getClass().getSimpleName() + (name != null ? " " + name : "");
        }
        if (obj instanceof Identifiable) {
            return obj.getClass().getSimpleName() + " " + ((Identifiable) obj).getId();
        }
    ...
    public static String getTitle(Object obj) {
        ...
        // 检查映射启用开关
        if (mapping == null || !isMappingEnabled()) {
        ...
        return buildTitle(mapping, suffix);
    }
